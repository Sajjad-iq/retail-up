...
public interface SystemSettingRepository extends JpaRepository<SystemSetting, String> {
    ...
    List<SystemSetting> findByOrganizationId(String organizationId);

    // Find settings by organization ID and setting type
    List<SystemSetting> findByOrganizationIdAndSettingType(String organizationId, String settingType);

    // Find setting by organization ID and key
    Optional<SystemSetting> findByOrganizationIdAndKey(String organizationId, String key);
    ...
    List<SystemSetting> findByOrganizationIdAndIsDefaultTrue(String organizationId);

    // Find settings by organization ID and setting type that are not default
    List<SystemSetting> findByOrganizationIdAndSettingTypeAndIsDefaultFalse(String organizationId, String settingType);

    // Search settings by organization ID and key
    @Query("SELECT s FROM SystemSetting s WHERE s.organizationId = :organizationId AND s.key LIKE %:searchTerm%")
    List<SystemSetting> searchSettingsByKey(@Param("organizationId") String organizationId,
            @Param("searchTerm") String searchTerm);

    // Search settings by organization ID and setting type
    @Query("SELECT s FROM SystemSetting s WHERE s.organizationId = :organizationId AND s.settingType LIKE %:searchTerm%")
    List<SystemSetting> searchSettingsByType(@Param("organizationId") String organizationId,
            @Param("searchTerm") String searchTerm);
